...
typedef struct {
    ...
    volatile ulong *colors;  // long color of each particle (not used)
...
typedef struct {
    ...
    uint colorMap;                 // which color map we use
} ImageData;

typedef struct {
    // NOT USED IN CUDA
    volatile double *f_field;  // 2D array to return force vals without malloc
    volatile double *a_tot;    // 2D array to return accel vals without malloc
...
// simulation data passed to CUDA
typedef struct {
    volatile SimInfo *simInfo;      // global SimInfo struct
    volatile Field *field;          // global Field struct
    volatile Particles *particles;  // global Particles struct
