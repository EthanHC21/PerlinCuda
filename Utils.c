...
void *initializeParticles(void *ptr) {
    ...
    for (uint i = threadData->startIdx;
        ...
        // threadData->particles->xPos[i] =
        //     randDouble(0, threadData->simInfo->width);
        // threadData->particles->yPos[i] =
        //     randDouble(0, threadData->simInfo->height);
        // velocities are already set to 0

        threadData->particles->valid[i] = 1;

        uint y = i / threadData->simInfo->width;
        uint x = i - y * threadData->simInfo->height;
...
void linterp2(ThreadData *threadData, uint i) {
    // extract coords
...
void getAcceleration(ThreadData *threadData, uint i) {
    // field force
    linterp2(threadData, i);

    // apply field strength and convert from grids to pixels
    threadData->f_field[0] *=
        threadData->field->fieldStrength * threadData->field->scl;
    threadData->f_field[1] *=
        threadData->field->fieldStrength * threadData->field->scl;
    ...
    double f_drag[2] = {
        threadData->particles->xVel[i] * mag * -1 * threadData->particles->C_d,
        threadData->particles->yVel[i] * mag * -1 * threadData->particles->C_d};

    threadData->a_tot[0] =
        (threadData->f_field[0] + f_drag[0]) / threadData->particles->m;
    threadData->a_tot[1] =
        (threadData->f_field[1] + f_drag[1]) / threadData->particles->m;
}
...
void *verlet(void *ptr) {
    ...
    for (uint i = threadData->startIdx;
         i < (threadData->startIdx + threadData->num); i++) {
        if (threadData->particles->valid[i] == 1) {
            // get acceleration due to drag and field
            getAcceleration(threadData, i);

            // calculate v_(t/2)
            threadData->particles->xVel[i] += threadData->a_tot[0] * .5;
            threadData->particles->yVel[i] += threadData->a_tot[1] * .5;

            // calculate x_t with boundary check
            threadData->particles->xPos[i] = fmod(threadData->particles->xPos[i] + threadData->particles->xVel[i] + (double)threadData->simInfo->width, (double)threadData->simInfo->width);
            threadData->particles->yPos[i] = fmod(threadData->particles->yPos[i] + threadData->particles->yVel[i] + (double)threadData->simInfo->height, (double)threadData->simInfo->height);
...
void *updateVelocities(void *ptr) {
    ...
    for (uint i = threadData->startIdx;
        ...
        double xNoise =
            x * threadData->field->scl * threadData->field->noiseScl +
            threadData->field->scl / 2.0;
        double yNoise =
            y * threadData->field->scl * threadData->field->noiseScl +
            threadData->field->scl / 2.0;

        threadData->field->field[i] =
            open_simplex_noise3(threadData->simInfo->ctx, xNoise, yNoise,
                                threadData->simInfo->t) *
            2 * M_PI;
...
void drawParticles(ImageData *imageData, SimInfo *simInfo,
    ...
    uint val = ipow(2, 16) - 1;

    if (imageData->writeName == NULL) {
        // file name
        imageData->writeName = malloc(strlen(imageData->imgName) + 12 + 1);
    ...
    // write the frame num
    sprintf(imageData->writeName + strlen(imageData->imgName), "_%05d.tiff",
            simInfo->frameNum);

    // white at each particle
    for (uint64_t i = 0; i < (particles->numParticles); i++) {
        uint idx = ((uint)particles->xPos[i]) +
                   ((uint)particles->yPos[i]) * simInfo->width;
        imageData->img[idx] = val;
    }

    writeTiff16BW(imageData->img, imageData->writeName, simInfo->width,
                  simInfo->height);
}

void drawField(Field *field, SimInfo *simInfo) {
    uint16_t *write = malloc(simInfo->rows * simInfo->cols * sizeof(uint16_t));
    uint val = ipow(2, 16) - 1;
    ...
    writeTiff16BW(write, "out/noise.tiff", simInfo->cols, simInfo->rows);
    free(write);
}
...
void writeAccumulator(SimInfo *simInfo, ImageData *imageData) {
    ...
    // write array
    uint16_t *write = malloc(sizeof(uint16_t) * numPixels);

    // sort it
    mergeSort(imageData->countSorted, 0, numPixels - 1);

    double min = imageData->countSorted[0];
    double max = imageData->countSorted[(uint)(.95 * numPixels)];
    ...
    for (int i = 0; i < numPixels; i++) {
        ...
        write[i] = (uint16_t)(normVal * val);
    }

    writeTiff16BW(write, "out/accumulator.tiff", simInfo->width,
                  simInfo->height);
